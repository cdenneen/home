...

{
  ...
  programs = {
    ...
    awscli = {
      ...
      credentials = {
        nextology = {
          credential_process = ''sh -c "OP_SERVICE_ACCOUNT_TOKEN=$(cat ${config.sops.secrets.op_service_account_token_ap.path}) op --vault=cdenneen-env item get --format=json --fields=label=AccessKeyId,label=SecretAccessKey nextology | jq 'map({key: .label, value: .value}) | from_entries + {Version: 1}'"'';
        };
      };
    };
