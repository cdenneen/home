...
def classify_issue(
    ...
    if issue.get("state") == "closed":
        ...
        return "Completed", None, "closed without an implementation MR"
...
def main() -> int:
    ...
    active_assignment_records = [
        item for item in assignment_records if item.get("state") not in {"complete", "completed", "cancelled"}
    ]
    active_leases = []
    ...
    idle_proof = {
        ...
        "all_waiting_items_decomposition_evaluated": waiting_decomposition_complete,
    ...
    idle_proof["queue_zero_proven"] = bool(
        not executable_queue
        and idle_proof["all_discovered_items_classified"]
        and idle_proof["all_configured_repositories_inspected"]
        and idle_proof["unknown_count"] == 0
        and idle_proof["dependency_query_failures"] == 0
        and idle_proof["all_waiting_items_decomposition_evaluated"]
        and idle_proof["blocked_items_isolated"]
        and idle_proof["running_count"] == 0
        and idle_proof["active_assignment_count"] == 0
        and idle_proof["active_lease_count"] == 0
    )
