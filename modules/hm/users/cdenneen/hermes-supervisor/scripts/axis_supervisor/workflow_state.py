...
class WorkflowState:
    ...
    def load_queue(self) -> dict:
        if self.queue_path.exists():
            try:
                ...
                if legacy.get("schema_version") not in {"1.0.0", "2.0.0", "3.0.0"}:
                    raise
                for item in legacy.get("items") or []:
                    ...
                    item.setdefault("dispatch_generation", None)
                    item.setdefault(
                        "delivery_lane",
                        {
                            "awaiting-review": "REVIEW",
                            "waiting-ci": "INTEGRATION",
                            "ready": "MERGE_READY",
                            "integrating": "INTEGRATION",
                            "integrated": "POST_MAIN_VERIFICATION",
                            "blocked": "REPAIR",
                        }.get(str(item.get("state")), "REVIEW"),
                    )
    ...
    def adapt_handoff(self, handoff: dict) -> dict:
        ...
        if value.get("schema_version") in {"1.0.0", "2.0.0", "3.0.0"}:
            ownership = resolve_repository_ownership(
            ...
            value.setdefault("dispatch_generation", None)
            value.setdefault("delivery_lane", "HANDOFF")
        validate_record(value, HANDOFF_SCHEMA)
        return value
