...
}:
{
  ...
  config = lib.mkMerge [
    {
      # Required by nix-darwin for all configurations
      system.stateVersion = 6;
      # Determinate manages Nix on macOS; disable nix-darwin Nix management
      nix.enable = false;
    ...
    (lib.mkIf config.profiles.defaults.enable {
      ...
      system = {
        ...
        primaryUser = config.userPresets.cdenneen.name;
      };
